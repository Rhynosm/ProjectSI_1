      ...
      <div class="row">
        ...
        <div class="container mt-5">
          <div class="row">
              <div class="col-md-12">
                  <div>
                      <h3 class="text-center my-4">DATA PENDIDIKAN PEGAWAI</h3>    
                      <hr>
                  </div>
                  <div class="card border-0 shadow-sm rounded">
                      <div class="card-body">
                          <a href="{{ route('postq.create') }}" class="btn btn-md btn-success mb-3">TAMBAH POST</a>
                          <a href='{{ url('postq/view/pdf') }}' class="btn btn-danger fa fa-download ml-5">Download PDF</a>
                          <table class="table table-bordered">
                              <thead>
                                <tr>
                                  <th scope="col">PENDIDIKAN TERAKHIR</th>
                                  <th scope="col">NAMA INSTITUSI PENDIDIKAN</th>
                                  <th scope="col">TAHUN LULUS</th>
                                  <th scope="col">NILAI GPA</th>
                                  <th scope="col">ACTION</th>
                                </tr>
                              </thead>
                              <tbody>
                                @forelse ($postq as $postw)
                                  <tr>
                                      <td>{{ $postw->pendtera }}</td> 
                                      <td>{{ $postw->namainstitusi }}</td>
                                      <td>{{ $postw->tahunlulus }}</td>
                                      <td>{{ $postw->gpa }}</td>
                                      <td class="text-center">
                                          <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('postq.destroy', $postw->id) }}" method="POST">
                                              <a href="{{ route('postq.show', $postw->id) }}" class="btn btn-sm btn-dark">SHOW</a>
                                              <a href="{{ route('postq.edit', $postw->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                              @csrf
                                              @method('DELETE')
