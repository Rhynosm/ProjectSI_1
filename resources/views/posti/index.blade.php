      ...
      <div class="row">
        ...
        <div class="container mt-5">
          <div class="row">
              <div class="col-md-12">
                  <div>
                      <h3 class="text-center my-4">DATA PENGAJUAN IJIN PEGAWAI</h3>    
                      <hr>
                  </div>
                  <div class="card border-0 shadow-sm rounded">
                      <div class="card-body">
                          <a href="{{ route('posti.create') }}" class="btn btn-md btn-success mb-3">TAMBAH POST</a>
                          <a href='{{ url('posti/view/pdf') }}' class="btn btn-danger fa fa-download ml-5">Download PDF</a>
                          <table class="table table-bordered">
                              <thead>
                                <tr>
                                  <th scope="col">STATUS IJIN</th>
                                  <th scope="col">KETERANGAN IJIN</th>
                                  <th scope="col">DURASI IJIN</th>
                                  <th scope="col">ACTION</th>
                                </tr>
                              </thead>
                              <tbody>
                                @forelse ($posti as $postx)
                                  <tr>
                                      <td>{{ $postx->status }}</td> 
                                      <td>{{ $postx->alasan }}</td>
                                      <td>{{ $postx->durasi }}</td>
                                      <td class="text-center">
                                          <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('posti.destroy', $postx->id) }}" method="POST">
                                              <a href="{{ route('posti.show', $postx->id) }}" class="btn btn-sm btn-dark">SHOW</a>
                                              <a href="{{ route('posti.edit', $postx->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                              @csrf
                                              @method('DELETE')
