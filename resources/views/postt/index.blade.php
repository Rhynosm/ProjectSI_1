      ...
      <div class="row">
        ...
        <div class="container mt-5">
          <div class="row">
              <div class="col-md-12">
                  <div>
                      <h3 class="text-center my-4">DATA TUNJANGAN PEGAWAI</h3>    
                      <hr>
                  </div>
                  <div class="card border-0 shadow-sm rounded">
                      <div class="card-body">
                          <a href="{{ route('postt.create') }}" class="btn btn-md btn-success mb-3">TAMBAH POST</a>
                          <a href='{{ url('postt/view/pdf') }}' class="btn btn-danger fa fa-download ml-5">Download PDF</a>
                          <table class="table table-bordered">
                              ...
                              <tbody>
                                @forelse ($postt as $postu)
                                  <tr>
                                      <td>{{ $postu->namatunjangan }}</td> 
                                      <td>{{ $postu->keuntungan }}</td>
                                      <td>{{ $postu->tanggalmulai }}</td>
                                      <td>{{ $postu->tanggalberakhir }}</td>
                                      <td class="text-center">
                                          <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('postt.destroy', $postu->id) }}" method="POST">
                                              <a href="{{ route('postt.show', $postu->id) }}" class="btn btn-sm btn-dark">SHOW</a>
                                              <a href="{{ route('postt.edit', $postu->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                              @csrf
                                              @method('DELETE')
